...
@section('content')
    <section class="bg-white p-5 shadow border border-gray-300 rounded-lg mb-5">
        <form action="{{ route('admin.store-karyawan') }}" method="post" enctype="multipart/form-data" id="formTambahKaryawan">
            @csrf
            @method('POST')
            <div class="mb-4">
                <label for="nama" class="block mb-1 font-medium text-[16px] text-[#334155]">Nama</label>
                <input type="text" name="nama" id="nama"
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200" required>
                @error('nama')
